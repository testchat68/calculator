...
A retro-styled desktop calculator for **Linux Mint 22.2**, built entirely with Python's standard library (Tkinter — no `pip install` needed). It features a two-tier display (full expression on top, current value below), two independent memory slots (M1 / M2), thousands separators, and a warm 1962-inspired color palette.
...
4. Right-click on the `Calculator.desktop` file → **Properties → Permissions**  
   and check all **Execute** boxes.

That's it — now you can use the program.

Double-click it to launch the calculator like any other app.
...
**Forever Free.** This project is free to use, copy, and modify — for private, public, or business purposes — with no restrictions and no warranty.
