...
до следващия от същата група или до C.

RM1 / RM2:
  единичен клик — припомня паметта
  двоен клик     — нулира съответната памет (M1 или M2)
"""
...
class Calculator:
    def __init__(self, root):
        ...
        # Нова логика след "=":
        # запазва първия операнд и оператора, а нововъведеното число
        # става нов втори операнд. Например 2+2=4, 8 → 10, 26 → 28.
        self.repeat_equal = False
        self.repeat_left = None
        self.repeat_op = None
        self.repeat_typing = False

        # таймер за разграничаване на единичен / двоен клик върху RM1/RM2
        self._rm_timer = None
    ...
    def create_widgets(self):
        ...
        # ── Ред 0 ───────────────────────────────────────────────
        self.btn_c = self.make_btn(buttons_frame, "C",   0, 0, MAGENTA,  fg=WHITE)
        self.make_btn(buttons_frame, "⌫",   0, 1, MAGENTA,  fg=WHITE)
        self.make_btn(buttons_frame, "√",   0, 2, KEY_SOFT, fg=WHITE)
        self.make_btn(buttons_frame, "%",   0, 3, KEY_SOFT, fg=WHITE)   # ← беше ÷
        ...
        self.make_btn(buttons_frame, "÷",   1, 3, PINK,     fg=WHITE)   # ← слезе от (0,3)
        ...
        self.make_btn(buttons_frame, "×",   2, 3, PINK,     fg=WHITE)   # ← слезе от (1,3)
        ...
        self.make_btn(buttons_frame, "−",   3, 3, PINK,     fg=WHITE)   # ← слезе от (2,3)
        ...
        self.make_btn(buttons_frame, "=",   4, 2, PINK,     fg=WHITE)   # ← беше % на (4,2)
        self.make_btn(buttons_frame, "+",   4, 3, MAGENTA,  fg=WHITE)   # ← слезе от (3,3), цвят като C
        self.make_btn(buttons_frame, "±",   4, 4, TEAL,     fg=WHITE)   # ← цвят като паметта

        for i in range(5):
            buttons_frame.grid_rowconfigure(i, weight=1)
            buttons_frame.grid_columnconfigure(i, weight=1)

        # ── Специално поведение на RM1 / RM2 ────────────────────
        # Единичен клик → припомняне (с леко закъснение, за да се различи от двоен)
        # Двоен клик     → нулиране на съответната памет
        self.btn_m1_rcl.config(command=lambda: self._rm_single_click("RM1"))
        self.btn_m1_rcl.bind("<Double-Button-1>", lambda e: self._rm_double_click(1))
        self.btn_m2_rcl.config(command=lambda: self._rm_single_click("RM2"))
        self.btn_m2_rcl.bind("<Double-Button-1>", lambda e: self._rm_double_click(2))

    # ── Помощни за RM1 / RM2 ────────────────────────────────────
    def _rm_single_click(self, rm_key):
        if self._rm_timer is not None:
            self.root.after_cancel(self._rm_timer)
        self._rm_timer = self.root.after(250, lambda: self._rm_do_recall(rm_key))

    def _rm_do_recall(self, rm_key):
        self._rm_timer = None
        self.on_button_click(rm_key)

    def _rm_double_click(self, which):
        if self._rm_timer is not None:
            self.root.after_cancel(self._rm_timer)
            self._rm_timer = None
        if which == 1:
    ...
    # ── Останалата част от класа — непроменена ─────────────────
    ...
    # ── Повторение на операцията след "=" ─────────────────────────
    def arm_equal_repeat(self, expr):
        """Запомня първия операнд и оператора от израза преди '='."""
        m = re.search(r"([+−×÷])", expr)
        if not m:
            self.repeat_equal = False
            self.repeat_left = None
        ...
        self.repeat_left = left
        self.repeat_op = m.group(1)
        self.repeat_equal = True
        self.repeat_typing = False

    def clear_equal_repeat(self):
        self.repeat_equal = False
        self.repeat_left = None
        self.repeat_op = None
        self.repeat_typing = False

    def repeat_with_new_operand(self, digit):
        """Започва/продължава новия втори операнд след '='.

        Самото число се набира нормално. Изчислението става при следващия '='.
        Така работи например:
            2+2=  → 4
            8=    → 10
            26=   → 28
        """
        if self.repeat_left is None or self.repeat_op is None:
            return False

        if not self.repeat_typing:
            # Първата цифра започва нов втори операнд.
            self.expression = f"{self.repeat_left}{self.repeat_op}{digit}"
            self.repeat_typing = True
        else:
            self.expression += digit

        self.just_evaluated = False
        self.last_action = self.expression
        self.display_var.set(self.last_number(self.expression))
        return True
    ...
    def last_number(self, text):
        if not text or text in ("Error", "Error"):
            return text or "0"
        parts = re.split(r"([+−×÷])", text)
    ...
    def refresh_display(self):
        ...
        if self.repeat_typing:
            self.expr_display.config(text=self.format_for_screen(self.expression))
    ...
    def on_button_click(self, char):
        ...
        if char == "C":
            ...
            self.last_action = ""
            self.just_evaluated = False
            self.clear_equal_repeat()
            self.display_var.set("0")
        elif self.repeat_equal and char.isdigit():
            self.repeat_with_new_operand(char)
        elif self.repeat_equal and char == ".":
            if not self.repeat_typing:
                self.repeat_with_new_operand("0.")
            else:
                # Не допускаме втора десетична точка в новия операнд.
                current = self.last_number(self.expression)
                if "." not in current:
                    self.repeat_with_new_operand(".")
        elif char == "⌫":
            if self.repeat_equal:
                self.clear_equal_repeat()
            self.just_evaluated = False
            self.last_action = ""
            ...
            try:
                ...
                self.arm_equal_repeat(raw_expr)
                self.just_evaluated = True
                self.expression = shown
                self.display_var.set(shown)
            except Exception:
